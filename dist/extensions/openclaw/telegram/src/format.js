...
_export(exports, {
    ...
    get markdownToTelegramChunks () {
        return markdownToTelegramChunks;
    },
    ...
    get renderTelegramHtmlText () {
        return renderTelegramHtmlText;
    },
    get splitTelegramHtmlChunks () {
        return splitTelegramHtmlChunks;
    },
    get telegramHtmlToPlainTextFallback () {
        return telegramHtmlToPlainTextFallback;
    },
    get wrapFileReferencesInHtml () {
 ...
 */ function buildTelegramLink(link, text) {
    ...
    if ((0, _textchunking.isAutoLinkedFileRef)(href, label)) {
        return null;
    }
...
function renderTelegramHtml(ir) {
    return (0, _textchunking.renderMarkdownWithMarkers)(ir, {
        styleMarkers: {
            bold: {
                open: "<b>",
                close: "</b>"
...
function markdownToTelegramHtml(markdown, options = {}) {
    const ir = (0, _textchunking.markdownToIR)(preserveTelegramListBoundarySpacing(markdown ?? ""), {
        linkify: true,
        enableSpoilers: true,
        headingStyle: "none",
        blockquotePrefix: "",
 ...
 */ /** Escape regex metacharacters in a string */ function escapeRegex(str) {
...
function isSupportedTelegramHtmlTag(rawTag) {
    ...
    const name = (0, _stringcoerceruntime.normalizeLowercaseStringOrEmpty)(match[2]);
    const attrs = match[3] ?? "";
...
function preserveTelegramHtmlTag(rawTag, openTags, escapeTag) {
    ...
    const closing = match[1] === "/";
    const tagName = (0, _stringcoerceruntime.normalizeLowercaseStringOrEmpty)(match[2]);
    const attrs = match[3] ?? "";
...
function telegramHtmlToPlainTextFallback(html) {
    ...
    const withPlainLinks = html.replace(TELEGRAM_HTML_ANCHOR_PATTERN, (_match, doubleQuotedHref, singleQuotedHref, unquotedHref, labelHtml)=>{
...
function getFileReferencePattern() {
    ...
    const fileExtensionsPattern = Array.from(_textchunking.FILE_REF_EXTENSIONS_WITH_TLD).map(escapeRegex).join("|");
...
function wrapFileReferencesInHtml(html) {
    ...
    const deLinkified = html.replace(AUTO_LINKED_ANCHOR_PATTERN, (_match, label)=>{
        if (!(0, _textchunking.isAutoLinkedFileRef)(`http://${label}`, label)) {
            return _match;
        }
...
const TELEGRAM_SELF_CLOSING_HTML_TAGS = new Set([
    "br"
]);
...
function renderTelegramChunksWithinHtmlLimit(ir, limit) {
    return (0, _textchunking.renderMarkdownIRChunksWithinLimit)({
        ir,
        limit,
