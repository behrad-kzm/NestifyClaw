...
const _qrimage = require("./qr-image.js");
...
function resolveWhatsAppLoginTimeoutMs(value, fallbackMs, minMs) {
    return (0, _numberruntime.resolveTimerTimeoutMs)(value, fallbackMs, minMs);
}
function closeSocket(sock) {
    (0, _connectioncontroller.closeWaSocket)(sock);
}
...
async function ensureQrDataUrl(params) {
    ...
    if (current?.id !== params.loginId || current.qrVersion !== params.qrVersion || current.qr !== params.qr) {
        return await (0, _qrimage.renderQrPngDataUrl)(params.qr);
    }
    ...
    const renderPromise = (async ()=>{
        for(let attempt = 0; attempt < MAX_QR_RENDER_CHASES; attempt += 1){
            ...
            const dataUrl = await (0, _qrimage.renderQrPngDataUrl)(qr);
...
function attachLoginWaiter(accountId, login) {
    login.waitPromise = (0, _connectioncontroller.waitForWhatsAppLoginResult)({
        sock: login.sock,
...
async function startWebLoginWithQr(opts = {}) {
    ...
    const authState = await (0, _session.readWebAuthExistsForDecision)(account.authDir);
    if (authState.outcome === "unstable") {
        return {
            code: _session.WHATSAPP_AUTH_UNSTABLE_CODE,
    ...
    let sock;
    let pendingQr = null;
    const loginId = (0, _nodecrypto.randomUUID)();
    try {
        sock = await (0, _session.createWaSocket)(false, Boolean(opts.verbose), {
            ...
            onQr: (qr)=>{
                ...
                runtime.log((0, _runtimeenv.info)("WhatsApp QR received."));
            }
        });
    } catch (err) {
        clearTimeout(qrTimer);
    ...
    while(true){
        if (login.error) {
            if (login.errorStatus === 401) {
                const message = _connectioncontroller.WHATSAPP_LOGGED_OUT_QR_MESSAGE;
                await resetActiveLogin(account.accountId, message);
                runtime.log((0, _runtimeenv.danger)(message));
                return {
                    connected: false,
                    message
                };
        ...
        if (login.connected) {
            ...
            runtime.log((0, _runtimeenv.success)(message));
            await resetActiveLogin(account.accountId);
            return {
                connected: true,
                message
